...
public class BorrowBookController implements Initializable{
    ...
    @FXML
    void btnCheckoutClicked(ActionEvent event) {
    	...
    	try {
    		...
    		else
    		{
	            ...
	            CallableStatement cStatement = dbConnection.prepareCall("{call usp_BorrowBook(?,?,?,?)}");
	            ...
	            cStatement.setDate(3, java.sql.Date.valueOf(LocalDate.now()));
	            cStatement.setDate(4, null);
	            ((SQLServerCallableStatement)cStatement).execute();
	            dbConnection.commit();
	            dbConnection.close();
	            Alert alert =new Alert(AlertType.INFORMATION);
	            alert.setContentText("Borrowed book successfully");
	            alert.showAndWait();
    		}
    	}
    	...
    	try {
    		...
    		else
    		{
    			...
    			else
    			{
        			...
        			booksStatement.setString(1, comboBoxBranch.getSelectionModel().getSelectedItem());
        			...
        			else 
        			{
            			do
            			{
            				listViewBooks.getItems().add(rsBooks.getString("BookID") + ", " + rsBooks.getString("BookTitle") + ", " + rsBooks.getString("Author"));
            			}while (rsBooks.next()); 
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try {
    		...
    		else
    		{
				...
				while(rs.next())
    			{
    				comboBoxBranch.getItems().add(rs.getString("BranchName") );
    			}
